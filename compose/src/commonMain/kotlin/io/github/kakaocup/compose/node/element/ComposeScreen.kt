...
open class ComposeScreen<out T : ComposeScreen<T>> : BaseNode<T> {
    ...
    companion object {
        ...
        ): T = instantiateScreen<T>(semanticsProvider).apply { this(function) }

        inline fun <reified T : ComposeScreen<T>> onComposeScreen(
            noinline function: T.() -> Unit,
        ): T = instantiateScreen<T>().apply { this(function) }
    }
}

@PublishedApi
internal expect inline fun <reified T : ComposeScreen<T>> instantiateScreen(
    semanticsProvider: SemanticsNodeInteractionsProvider,
): T

@PublishedApi
internal expect inline fun <reified T : ComposeScreen<T>> instantiateScreen(): T
